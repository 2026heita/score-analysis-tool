...
当前主分析链路不再依赖固定的“学生、成绩、排名”字段，而是采用领域无关的通用字段模型，根据数据内容和字段名识别：
...
通用（generic）模式有以下明确边界：

- 不依赖任何教育字段语义；任意数值字段不会仅因取值落在 0~150 范围、或字段名是中文，就被解释成“成绩”。
- 教育（education）规则仅存在于 legacy 兼容层，用于历史成绩数据；主链路（App / components / engine / metrics / general schema）不直接消费 `courseScore` / `primaryTotal` / `rank` 等旧角色。
- 指标方向与角色分离：`lower_is_better` 不等于“排名”。“响应耗时 / 库存周转天数 / 成本”等指标即使方向设为越低越好，角色仍是 `metric`，不会被当作 `rank`。
- 指标方向默认不自动判定（`unspecified`），用户可明确设置“越高越好 / 越低越好 / 中性”。
...
内置示例数据均为 synthetic / 模拟数据，不涉及真实客户或品牌，用于体验趋势、分组、相关性与异常候选分析。其中重点项目为"零售数仓销售数据"：包含日期、门店、区域、商品、品类、渠道、销量、订单数、销售额、成本、毛利和折扣等字段，覆盖时间、标识、维度与指标多类字段，可同时演示时间趋势、分组、筛选、相关性与异常候选能力。
