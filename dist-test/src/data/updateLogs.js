 ...
 */
export const updateLogs = Object.freeze(_updateLogsData);
// ─── DEV 环境防误用检测 + 内容边界校验 ─────────────────────────
if (import.meta.env.DEV) {
    ...
    for (let i = 0; i < updateLogs.length; i++) {
        ...
        for (const term of forbiddenTerms) {
            if (entry.title.toLowerCase().includes(term.toLowerCase())) {
                console.warn(`${prefix}: title 包含禁止术语 "${term}"。` +
                    `用户公告应使用用户可理解的语言。`);
            }
        }
        // 检查 items
        for (let j = 0; j < entry.items.length; j++) {
            for (const term of forbiddenTerms) {
                if (entry.items[j].toLowerCase().includes(term.toLowerCase())) {
                    console.warn(`${prefix}: items[${j}] 包含禁止术语 "${term}"。` +
                        `用户公告应使用用户可理解的语言。`);
                }
            }
        }
    ...
    for (const method of mutationMethods) {
        ...
        Array.prototype[method] = function (...args) {
            if (this === updateLogs) {
                console.error(`[updateLogs] ❌ IMMUTABLE VIOLATION: Array.prototype.${method}() called on frozen updateLogs. ` +
                    `Use createUpdateLogEntry() to create a new version instead.`);
                throw new TypeError(`updateLogs is immutable. Cannot call ${method}(). Use createUpdateLogEntry() instead.`);
            }
            // 对 entry.items 也做检测
            for (const entry of updateLogs) {
                if (this === entry.items) {
                    console.error(`[updateLogs] ❌ IMMUTABLE VIOLATION: Array.prototype.${method}() called on frozen entry.items ` +
                        `("${entry.version}"). Entries are immutable.`);
                    throw new TypeError(`updateLogs entry items are immutable. Cannot call ${method}() on "${entry.version}" items.`);
                }
            }
 ...
 * 用于开发时检查数据结构是否正确
 */
export function validateUpdateLogs(logs) {
    const errors = [];
    ...
    for (let i = 0; i < logs.length; i++) {
        ...
        if (!log.version)
            errors.push(`${prefix}: version 不能为空`);
        if (!log.title)
            errors.push(`${prefix}: title 不能为空`);
        if (!log.items || log.items.length === 0)
            errors.push(`${prefix}: items 不能为空`);
        // 检查 version 唯一性
 ...
 * - type 字段合法性
 */
export function validateReleaseAnnouncement(entry) {
    const warnings = [];
    const errors = [];
    // 结构检查
    if (!entry.version)
    ...
    if (!entry.items || entry.items.length === 0) {
        ...
        if (entry.items.length < 3)
            warnings.push(`items 数量 ${entry.items.length} < 3，公告过于碎片化`);
        if (entry.items.length > 6)
            warnings.push(`items 数量 ${entry.items.length} > 6，公告过度膨胀`);
    }
    // type 合法性检查
 ...
 * });
 */
export function createUpdateLogEntry(logs, newEntry) {
    // 检查 version 是否已存在
    const existingVersion = logs.find(log => log.version === newEntry.version);
    if (existingVersion) {
        throw new Error(`Version "${newEntry.version}" already exists in updateLogs. ` +
