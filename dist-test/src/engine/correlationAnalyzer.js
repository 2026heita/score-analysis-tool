 ...
 * 3. 不是高唯一性长数字 ID
 */
function isAnalyzableNumericField(feature, fieldMeta) {
    // 必须是数值型
 ...
 */
function isHighUniqueLongNumberId(header, values, fieldMeta) {
    // 检查字段名关键词
 ...
 * 对字段强度分类
 */
function classifyStrength(absR, strongThreshold, moderateThreshold) {
    if (absR >= strongThreshold)
        return 'strong';
    if (absR >= moderateThreshold)
        return 'moderate';
    return 'weak';
}
 ...
 */
export function analyzeCorrelations(headers, rows, features, config = {}, fieldMetas) {
 ...
 */
export function analyzeCorrelationsSimple(headers, rows, config = {}) {
    // ID 字段关键词（用于简单版过滤）
    const idKeywords = ['学号', '考号', '考生号', '准考证', '身份证号', '编号', 'ID', 'id', '订单', '用户', '商品', '编码'];
    ...
    // 构造最小 features
    const features = numericalFields.map(h => ({
        fieldName: h,
        displayName: h,
        featureType: 'numerical',
        confidence: 0.9,
