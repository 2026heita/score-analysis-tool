 ...
 * @returns 分析结果
 */
export function analyzeDataset(headers, rows, config = {}) {
    ...
    for (const feature of features) {
        ...
        if (feature.featureType === 'numerical') {
            ...
            if (stats.validCount > 0) {
                insights.push(`字段 "${feature.fieldName}": ${stats.validCount} 个有效值, ` +
                    `范围 [${stats.min}, ${stats.max}], ` +
                    `均值 ${stats.mean}, 标准差 ${stats.std}`);
                if (outliers.length > 0) {
                    insights.push(`字段 "${feature.fieldName}" 检测到 ${outliers.length} 个异常值`);
                }
            }
        }
        // 类别字段分析
        if (feature.featureType === 'categorical') {
            ...
            insights.push(`字段 "${feature.fieldName}": ${valueCounts.size} 个类别, ` +
        ...
        if (feature.featureType === 'temporal') {
            ...
            if (timestamps.length > 0) {
                ...
                insights.push(`字段 "${feature.fieldName}": 时间范围 ${minDate} 到 ${maxDate}`);
            }
        }
    }
    // 5. 生成警告
 ...
 * 获取数据集 Schema
 */
export function getDatasetSchema(headers, rows) {
    return detectDatasetSchema(headers, rows);
}
/**
 * 标准化数据
 */
export function standardizeData(rows, features) {
...
export { analyzeNumericalFeature, calculatePercentile, calculateZScore, detectOutliers, } from './univariateAnalyzer';
