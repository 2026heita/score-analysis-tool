 ...
 */
export function extractFieldValues(rows, fieldName, _config = {}) {
 ...
 * @deprecated 仅供 computeMetric() 内部使用，外部应通过 computeMetric() 统一入口。
 */
export function computeStats(values, totalRows) {
 ...
 * @deprecated 仅供 computeMetric() 内部使用，外部应通过 computeMetric() 统一入口。
 */
export function computePosition(values, inputValue, direction = 'higher-is-better') {
 ...
 * @returns 百分位（0-100）
 */
export function computePercentile(values, inputValue, isRankField = false) {
 ...
 * @returns 是否为排名字段
 */
export function isRankField(fieldName, analysisRole) {
    // 优先使用 analysisRole
 ...
 * @returns 是否为数值字段
 */
export function isNumericField(rows, fieldName, config = {}) {
 ...
 * @returns 分析结果
 */
export function analyzeField(rows, fieldName, inputValue, config = {}) {
    // 1. 提取字段值
 ...
 * @returns 字段分析结果映射
 */
export function analyzeMultipleFields(rows, fieldNames, config = {}) {
 ...
 * @returns 指标计算结果
 */
export function computeMetric(context, metricDef, userValue) {
    // 1. 提取字段值（使用统一的截断和过滤逻辑）
    const { values, invalidCount, totalRows, truncatedRows } = extractFieldValues(context.filteredRows, metricDef.sourceField);
    // 2. 计算统计指标
    const stats = computeStats(values, truncatedRows);
    // 3. 计算位置（如果有用户输入值）
    let position;
 ...
 * @returns 指标结果映射
 */
export function computeMetrics(context, metricDefs, userValues) {
    const results = new Map();
