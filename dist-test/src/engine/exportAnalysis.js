 ...
 */
export function exportGroupStatsToCsv(groupStats) {
    const header = buildCsvLine(['维度值', '数量', '均值', '中位数', '最小值', '最大值', 'Q25', 'Q75']);
    const rows = groupStats.map(gs => buildCsvLine([
        gs.dimensionValue,
        gs.count,
        gs.mean,
        gs.median,
 ...
 * 保留原始字段顺序，首行为表头
 */
export function exportFilteredRowsToCsv(rows, headers) {
    const header = buildCsvLine(headers);
    const dataRows = rows.map(row => buildCsvLine(headers.map(h => row[h] ?? '')));
 ...
 * 包含：统计指标 + 排名定位 + 抽样信息
 */
export function exportSummaryToCsv(metricResult, stats, position, fieldName, samplingInfo) {
    const lines = [];
    // 抽样信息部分（Stage 0A-2）
    if (samplingInfo) {
