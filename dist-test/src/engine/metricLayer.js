 ...
 * @returns 语义层定义
 */
export function buildSemanticDefinitions(fieldMetas) {
    const metrics = [];
    const entities = [];
    const dimensions = [];
 ...
 * 获取所有推荐的 metrics
 */
export function getRecommendedMetrics(semantic) {
    return semantic.metrics.filter(m => m.isRecommended);
}
/**
 * 获取所有 rank 类型的 metrics
 */
export function getRankMetrics(semantic) {
    return semantic.metrics.filter(m => m.type === 'rank');
}
/**
 * 获取所有 entity 类型的定义
 */
export function getEntities(semantic) {
    return semantic.entities;
}
/**
 * 获取主键 entity
 */
export function getPrimaryKeyEntity(semantic) {
