 ...
 * 生成稳定的 datasetKey
 */
function generateDatasetKey(dataRevision, filterRevision) {
    return `${dataRevision}-${filterRevision}`;
}
/**
 * 创建分析数据集
 */
export function useAnalysisDataset(filteredParsedData, dataRevision, filterRevision) {
    const [confirmedDatasetKey, setConfirmedDatasetKey] = useState(null);
    const [cancelledDatasetKey, setCancelledDatasetKey] = useState(null);
    const datasetKey = useMemo(() => generateDatasetKey(dataRevision, filterRevision), [dataRevision, filterRevision]);
    // 数据变化时清除旧的确认/取消状态
    useEffect(() => {
    ...
    const confirmDataset = useCallback((key) => {
        setConfirmedDatasetKey(key);
        setCancelledDatasetKey(null);
    }, []);
    const cancelDataset = useCallback((key) => {
