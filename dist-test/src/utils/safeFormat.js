 ...
 * @returns 格式化后的字符串
 */
export function safeFormatNumber(value, decimals = 1, fallback = '—') {
 ...
 */
export function safeFormatPercent(value, decimals = 1, fallback = '—') {
    if (!isFiniteNumber(value)) {
        return fallback;
    }
    // 限制小数位数在 0-20 范围内，避免 RangeError
 ...
 */
export function extractNumericFromEChartsParam(param) {
 ...
 * 用于过滤无效的分析结果
 */
export function isValidPercentile(value) {
