 ...
 */
export async function parseWorkbook(arrayBuffer, _fileName, targetSheetName) {
    ...
    // 限制列数并包装为统一格式
    const sheetsData = rawSheets.map(s => ({
        name: s.name,
        data: s.data.map(row => {
 ...
 * 解析单个 sheet 的数据
 */
function parseSheetData(sheetName, rawData, merges, allCandidates) {
    // 保存物理总行数（截断前）
    ...
    let headerRowRangeText;
    let dataStartRowText;
