...
// ============================================================
import { detectAndFlattenMultiRowHeaders } from './headerFlattener';
// ============================================================
// 常量
// ============================================================
...
// 表头检测主函数（支持多级表头）
// ============================================================
export function detectHeaderRow(rawRows, merges) {
...
function isInvalidFieldName(name) {
    ...
    if (digitCount / withoutSeparators.length > 0.8)
        return true;
    return false;
}
/**
 * 原有的单行表头检测逻辑
 */
function detectSingleHeaderRow(rawRows) {
...
function rowHasManyNumbers(row) {
    if (!row || row.length === 0)
        return 0;
    return row.filter(v => {
