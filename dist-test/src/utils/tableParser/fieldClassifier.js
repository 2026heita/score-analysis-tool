 ...
 */
export function classifyFields(headers, rows) {
...
function classifyFieldByContent(header, columnValues, feature, rowCount) {
    ...
    if (feature.numericRatio > 0.5) {
        ...
        if (feature.valuePattern === 'longNumber' &&
        ...
        // 2d. 内容像分数（数值在合理分数范围内）
        if (feature.valuePattern === 'scoreLike' &&
 ...
 * 基于内容特征微调置信度
 */
function adjustConfidence(type, baseConfidence, feature, _counts, total, header) {
...
function isInvalidHeaderName(headerLower) {
    // 空或全空白
    if (!headerLower || /^[\s]+$/.test(headerLower))
        return true;
    // 未命名字段
    if (headerLower.startsWith('未命名字段'))
        return true;
    // 全符号
    if (/^[\s_\-\.]+$/.test(headerLower))
        return true;
    // 类似 93_80、0_0、101.60_91.60、202409602096_202409602084 这种数据行拼接
    // 特征：包含下划线且两侧都是数字，或整体是数字+小数点+下划线
    if (/^\d+[\._]\d+/.test(headerLower) || /^\d+_\d+/.test(headerLower))
        return true;
    // 纯数字字段名（长度>=4）
    ...
    if (headerLower.length >= 6) {
        ...
        if (nonDigitNonAlpha.length / headerLower.length < 0.2)
            return true;
    }
    return false;
}
 ...
 */
function classifyAnalysisRole(header, type, _feature) {
    const headerLower = header.toLowerCase().trim();
    // 1. 未命名字段 → invalid
    if (isInvalidHeaderName(headerLower)) {
        return 'invalid';
 ...
 * 统计列值分布
 */
function countColumnValues(values) {
 ...
 * 置信度 < 0.55 的字段不进入默认推荐
 */
export function recommendAnalysisField(fieldMetas) {
 ...
 * 默认推荐只包含置信度 >= 0.55 的字段
 */
export function getAnalyzableFields(fieldMetas, showAll = false) {
    if (showAll) {
        return fieldMetas
 ...
 * @returns 分析评分结果
 */
export function calculateFieldAnalyticScore(meta, sampleValues) {
    ...
    if (!isAnalyzable) {
        ...
        if (numericRatio < 0.3)
            reasons.push('数值比例过低');
        if (uniquePenalty > 0.5)
            reasons.push('唯一率过高，疑似ID/序号');
        if (monotonicPenalty > 0.5)
            reasons.push('单调递增，疑似序号');
        if (nameSignal < 0.3)
            reasons.push(`字段语义为非分析型(${role})`);
 ...
 */
export function getFieldAnalyticScore(meta) {
 ...
 * - nameSignal: 0.10
 */
export const genericScoringStrategy = {
    name: 'generic',
 ...
 */
export const numericHeavyScoringStrategy = {
    name: 'numericHeavy',
    score(meta, sampleValues) {
        // 复用现有计算逻辑，但调整权重
        const base = calculateFieldAnalyticScore(meta, sampleValues);
        const b = base.breakdown;
        const score = b.numericRatio * 0.35 +
            b.variance * 0.30 +
