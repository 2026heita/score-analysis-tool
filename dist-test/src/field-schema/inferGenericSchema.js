...
exports.inferGenericFieldSchema = inferGenericFieldSchema;
// ============================================================
// 通用字段名规则（不包含教育特定关键词）
// ============================================================
/** 标识符字段关键词 */
 ...
 * @returns 推断的字段模式
 */
function inferGenericFieldSchema(header, columnValues, contentFeature) {
...
function inferByFieldName(headerLower) {
    // 1. 标识符字段
...
function inferByContentFeature(feature) {
    const reasons = [];
    // 1. 高数值比例 → 数值类型
 ...
 * 计算字段统计信息
 */
function computeFieldStatistics(columnValues) {
 ...
 */
function computeContentFeature(columnValues) {
