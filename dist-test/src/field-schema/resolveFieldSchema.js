...
exports.resolveFieldSchemas = resolveFieldSchemas;
exports.shouldAnalyzeField = shouldAnalyzeField;
exports.shouldGenerateDirectionEvaluation = shouldGenerateDirectionEvaluation;
exports.getFieldDisplayLabel = getFieldDisplayLabel;
const inferGenericSchema_1 = require("./inferGenericSchema");
const education_1 = require("./templates/education");
const legacyAdapter_1 = require("./legacyAdapter");
// ============================================================
// 主解析函数
// ============================================================
/**
 ...
 * @returns 解析后的最终字段模式
 */
function resolveFieldSchema(header, schema, config) {
    ...
    if (mode === 'education' && schema) {
        const templateResult = (0, education_1.applyEducationTemplate)(header, schema);
        if (templateResult) {
    ...
    // 优先级 4：旧系统兼容结果
    if (legacyFieldMeta) {
        const legacySchema = (0, legacyAdapter_1.mapLegacyFieldMetaToSchema)(legacyFieldMeta, columnValues);
        // 如果是教育模式，尝试应用模板
        if (mode === 'education') {
            const templateResult = (0, education_1.applyEducationTemplate)(header, legacySchema);
            if (templateResult) {
                return extractResolvedSchema(templateResult);
            }
        }
    ...
    // 优先级 5：从内容推断
    if (columnValues) {
        const inferredSchema = (0, inferGenericSchema_1.inferGenericFieldSchema)(header, columnValues, contentFeature);
        // 如果是教育模式，尝试应用模板
        if (mode === 'education') {
            const templateResult = (0, education_1.applyEducationTemplate)(header, inferredSchema);
            if (templateResult) {
                return extractResolvedSchema(templateResult);
            }
        }
 ...
 * 应用用户覆盖
 */
function applyUserOverride(schema) {
 ...
 * 提取解析后的字段模式
 */
function extractResolvedSchema(schema) {
    return {
        fieldId: schema.fieldId,
        sourceName: schema.sourceName,
 ...
 */
function resolveFieldSchemas(headers, schemas, config) {
 ...
 * 检查字段是否应该参与分析
 */
function shouldAnalyzeField(resolved) {
    // 忽略的字段不参与分析
    if (resolved.ignored)
        return false;
    // ignored 角色不参与分析
    if (resolved.analysisRole === 'ignored')
        return false;
    // unspecified 角色不参与分析（需要用户确认）
    if (resolved.analysisRole === 'unspecified')
        return false;
    return true;
}
/**
 * 检查字段是否应该生成优劣评价
 */
function shouldGenerateDirectionEvaluation(resolved) {
    // neutral 和 unspecified 不生成优劣评价
    if (resolved.metricDirection === 'neutral')
        return false;
    if (resolved.metricDirection === 'unspecified')
        return false;
    return true;
}
/**
 * 获取字段的显示标签
 */
function getFieldDisplayLabel(resolved) {
    const typeLabels = {
        number: '数值',
        category: '类别',
        datetime: '日期时间',
