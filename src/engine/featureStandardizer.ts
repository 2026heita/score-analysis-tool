...
export type StandardizedValue = 
  | { type: 'numerical'; value: number; original: string }
  | { type: 'categorical'; value: string; original: string }
  | { type: 'temporal'; value: number; original: string } // timestamp
  | { type: 'text'; value: string; original: string }
  | { type: 'identifier'; value: string; original: string }
  | { type: 'invalid'; value: null; original: string }
  | { type: 'missing'; value: null; original: string };

// 标准化配置
interface StandardizerConfig {
  // 数值解析选项
  allowCommaSeparator?: boolean; // 允许逗号作为千分位（默认 true）
  
  // 时间解析选项
  strictTemporalParse?: boolean; // 严格时间解析（默认 false）
}

const DEFAULT_STANDARDIZER_CONFIG: StandardizerConfig = {
...
export function standardizeRow(
  ...
  const values: Record<string, StandardizedValue> = {};
 ...
 * 提取类别字段的标准化值
 */
export function extractCategoricalValues(
  vectors: FeatureVector[],
  fieldName: string
): string[] {
  const values: string[] = [];
 ...
 */
export function extractTemporalValues(
  vectors: FeatureVector[],
  fieldName: string
): number[] {
  const values: number[] = [];
 ...
 * 判断值是否可参与统计分析
 */
export function isAnalyzable(sv: StandardizedValue): boolean {
 ...
 * 获取标准化值的原始字符串
 */
export function getOriginalValue(sv: StandardizedValue): string {
  return sv.original;
}
