...
interface DetectionConfig {
  // 数值型阈值
  minNumericRatio?: number;      // 最小数值比例（默认 0.7）
  
  // 标识符阈值
  minUniqueRatio?: number;       // 最小唯一值比例（默认 0.9）
  minIdentifierLength?: number;  // 最小标识符长度（默认 6）
  
  // 时间型检测
  maxTemporalParseFail?: number; // 最大时间解析失败数（默认 3）
  
  // 类别型阈值
  maxCategoryRatio?: number;     // 最大类别比例（唯一值/总数，默认 0.5）
  maxCategoryCount?: number;     // 最大类别数量（默认 50）
  
  // 文本型阈值
  minTextLength?: number;        // 最小文本长度（默认 50）
}
 ...
 * 检测单个字段的类型
 */
export function detectFeatureType(
  fieldName: string,
  values: string[],
