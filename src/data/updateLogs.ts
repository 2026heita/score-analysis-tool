...
const _updateLogsData: UpdateLogItem[] = [
  {
    date: '2026-09-19',
    version: 'v2.3.1',
    title: '修复图表首次空白、查找残留，记录定位更可靠',
    type: 'fix',
    items: [
      '修复首次打开图表时图表区可能显示空白、需要切换图表类型后才恢复的问题',
      '恢复图表切换时的动画效果，切换过程更自然',
      '切换数据或表格后，查找并填充功能自动回到初始状态，不再残留上一次的输入与结果',
      '查找记录更可靠：能识别更多字段命名与表格写法（如多行表头、合并标题、字段改名），即使没有学号等唯一列，也能用姓名和班级等组合定位到对应记录',
      '图表加载流程优化，首次展示更稳定'
    ]
  },
  {
    date: '2026-09-03',
    version: 'v2.3.0',
    title: '帮助系统上线，记录定位与筛选更可靠',
    type: 'feature',
    items: [
      '用户帮助正式上线：在字段识别、数据筛选、分组分析、相对位置、离群值、相关性、时间趋势、记录定位、外部数据源与 AI 智能分析等位置，都提供通俗易懂的说明，帮助理解“有什么用、什么时候用”',
      '说明展示更友好：简短的说明轻量显示在按钮旁，较长的说明在右侧独立专区查看，不遮挡正在操作的输入与控件',
      '记录定位能力增强：除了用编号等唯一定位字段，还支持用“日期”等时间字段定位到具体记录；同一天出现多条记录时，会列出由你选择查看哪一条',
      '数据筛选更稳定：筛选结果为空时不再影响原本的分析数据、图表与导出能力，避免筛空后分析链路失效，并给出清晰提示',
      '外部数据源体验优化：连接设置（服务地址、日期范围等）在刷新页面后仍能恢复，使用更可靠',
      '记录查找入口更清晰：入口名称与提示语更新，明确是按可唯一定位的字段查找记录，避免误解'
    ]
  },
  {
    date: '2026-08-31',
