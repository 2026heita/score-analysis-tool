...
const _updateLogsData: UpdateLogItem[] = [
  {
    date: '2026-08-17',
    version: 'v2.2.2',
    title: '解析与筛选更可靠、结果更准确',
    type: 'improvement',
    items: [
      '数据量较大的表格会得到更明确的提示：超过分析上限（20,000 行）时不再静默截取前一部分继续分析，而是明确提示需要精简数据',
      '数据解析更可靠：新数据解析失败时不会继续显示上一份数据的旧分析结果；本地保存失败会明确提示，不再误提示成功',
      '异常值处理更准确：排除异常值后各项统计与图表会同步更新，分析结果更真实',
      '筛选更准确：销售额、利润、库存等普通数字字段可正确执行大于、小于等条件，"为空/不为空"筛选与系统空值规则一致',
      '数值计算更稳定：百分号数据在各分析模块口径统一，极大数字参与统计时不再出现异常，含"统计/累计/合计"等词的正常文本不再被误删',
      '兼容性与安全性提升：xlsx/xls、多工作表、合并表头继续正常解析，多层表头下数据条数判断更准确；表格导出时对类公式样内容做了安全处理'
    ]
  },
  {
    date: '2026-08-16',
